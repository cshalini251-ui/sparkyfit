...
import { getAIServices, getActiveAiServiceSetting } from '@/services/aiServiceSettingsService';
...
const DraggableChatbotButton: React.FC = () => {
  ...
  useEffect(() => {
    // Position button near bottom-right corner with margin
    const margin = 20;
    const xPos = Math.max(0, window.innerWidth - margin - 70); // button width ~56px plus margin
    const yPos = Math.max(0, window.innerHeight - margin - 70);
  ...
  useEffect(() => {
    const checkAiProviders = async () => {
      if (!loading && user?.id) { // Only fetch if not loading and user is available
        try {
          const activeSetting = await getActiveAiServiceSetting();
          setHasAiProvider(!!activeSetting);
        } catch (error) {
          console.error("Failed to fetch active AI service:", error);
