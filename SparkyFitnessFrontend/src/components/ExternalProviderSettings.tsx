...
interface ExternalDataProvider { // Renamed interface
  ...
  provider_type: 'openfoodfacts' | 'nutritionix' | 'fatsecret' | 'wger' | 'mealie' | 'free-exercise-db' | 'calorieninjas'; // Added free-exercise-db, calorieninjas
...
const ExternalProviderSettings = () => { // Renamed component
  ...
  const [newProvider, setNewProvider] = useState({
    provider_name: '',
    provider_type: 'openfoodfacts' as 'openfoodfacts' | 'nutritionix' | 'fatsecret' | 'wger' | 'mealie' | 'free-exercise-db' | 'calorieninjas', // Added free-exercise-db, calorieninjas
    app_id: '',
    app_key: '',
    is_active: false,
  ...
  const loadProviders = async () => {
    ...
    try {
      ...
      setProviders(data.map((provider: any) => ({
        ...provider,
        provider_type: provider.provider_type as 'openfoodfacts' | 'nutritionix' | 'fatsecret' | 'wger' | 'mealie' | 'free-exercise-db' | 'calorieninjas' // Added free-exercise-db, calorieninjas
      })) || []);
    } catch (error: any) {
  ...
  const handleAddProvider = async () => {
    ...
    } else if (newProvider.provider_type === 'calorieninjas') {
      if (!newProvider.app_key) {
        toast({
          title: "Error",
          description: `Please provide an API Key for CalorieNinjas`,
          variant: "destructive"
        });
        return;
      }
    ...
    try {
      const data = await apiCall('/external-providers', { // Corrected API endpoint
        ...
        body: JSON.stringify({
          ...
          app_id: (newProvider.provider_type === 'mealie' || newProvider.provider_type === 'free-exercise-db' || newProvider.provider_type === 'wger' || newProvider.provider_type === 'calorieninjas') ? null : newProvider.app_id || null, // Only set app_id for non-mealie, free-exercise-db, wger, calorieninjas
      ...
      loadProviders();
      if (data && data.is_active && (data.provider_type === 'openfoodfacts' || data.provider_type === 'nutritionix' || data.provider_type === 'fatsecret' || data.provider_type === 'mealie' || data.provider_type === 'calorieninjas')) { // Only set default for food providers
  ...
  const handleUpdateProvider = async (providerId: string) => {
    ...
    const providerUpdateData: Partial<ExternalDataProvider> = { // Renamed interface
      ...
      app_id: (editData.provider_type === 'mealie' || editData.provider_type === 'free-exercise-db' || editData.provider_type === 'wger' || editData.provider_type === 'calorieninjas') ? null : editData.app_id || null, // Only set app_id for non-mealie, free-exercise-db, wger, calorieninjas
  ...
  const getProviderTypes = () => [
    ...
    { value: "calorieninjas", label: "CalorieNinjas / API-Ninjas" },
  ];
        ...
        <CardContent className="space-y-4">
          ...
          {showAddForm && (
            <form onSubmit={(e) => { e.preventDefault(); handleAddProvider(); }} className="border rounded-lg p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setNewProvider(prev => ({ ...prev, provider_type: value as 'openfoodfacts' | 'nutritionix' | 'fatsecret' | 'wger' | 'mealie' | 'free-exercise-db' | 'calorieninjas', app_id: '', app_key: '', base_url: '' }))} // Added free-exercise-db, reset base_url
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              {newProvider.provider_type === 'calorieninjas' && (
                <div>
                  <Label htmlFor="new_app_key">API Key</Label>
                  <Input
                    id="new_app_key"
                  ...
                  <p className="text-sm text-muted-foreground mt-1 col-span-2">
                    Get your API Key from the <a href="https://calorieninjas.com/" target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">CalorieNinjas Dashboard</a> or <a href="https://api-ninjas.com/" target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">API-Ninjas Dashboard</a>.
                  </p>
                </div>
              )}
                  ...
                  <div key={provider.id} className="border rounded-lg p-4">
                    {editingProvider === provider.id ? (
                      ...
                      <form onSubmit={(e) => { e.preventDefault(); handleUpdateProvider(provider.id); }} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <Select
                              ...
                              onValueChange={(value) => setEditData(prev => ({ ...prev, provider_type: value as 'openfoodfacts' | 'nutritionix' | 'fatsecret' | 'wger' | 'mealie' | 'free-exercise-db' | 'calorieninjas', app_id: '', app_key: '', base_url: '' }))} // Added free-exercise-db, reset base_url
                            >
                              <SelectTrigger>
                                <SelectValue />
                              </SelectTrigger>
                        ...
                        {editData.provider_type === 'calorieninjas' && (
                          <div>
                            <Label>API Key</Label>
                            <Input
                              type="password"
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div>
                            <p className="text-sm text-muted-foreground">
                              ...
                              {(provider.provider_type !== 'mealie' && provider.provider_type !== 'free-exercise-db' && provider.provider_type !== 'wger' && provider.provider_type !== 'calorieninjas') && provider.app_id && ` - App ID: ${provider.app_id.substring(0, 4)}...`}
                              {(provider.provider_type === 'mealie' || provider.provider_type === 'nutritionix' || provider.provider_type === 'fatsecret' || provider.provider_type === 'calorieninjas') && provider.app_key && ` - App Key: ${provider.app_key.substring(0, 4)}...`}
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
                            <Switch
