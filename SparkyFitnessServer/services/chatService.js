...
async function getActiveAiServiceSetting(authenticatedUserId, targetUserId) {
  try {
    let setting = await chatRepository.getActiveAiServiceSetting(targetUserId);
    if (!setting && process.env.GEMINI_API_KEY) {
      setting = {
        id: 'system-gemini-fallback',
        user_id: targetUserId,
        service_name: 'Google Gemini (System)',
        service_type: 'google',
...
`;
    ...
    switch (aiService.service_type) {
      ...
      case 'google':
        ...
        const isGoogleApiKey = aiService.api_key.startsWith('AIza') || aiService.api_key.startsWith('AQ.');
        const googleUrl = isGoogleApiKey
        ...
        response = await fetch(googleUrl, {
          ...
          headers: {
            'Content-Type': 'application/json',
            ...(!isGoogleApiKey && { 'Authorization': `Bearer ${aiService.api_key}` })
          },
...
async function generateReportRecommendations(reportSummary, serviceConfig, authenticatedUserId) {
  try {
    if (!serviceConfig) {
      throw new Error('AI service configuration is missing.');
    }
    const systemPrompt = `You are Sparky, an AI nutrition and wellness coach.
Analyze the user's fitness and health report data for the specified date range.
Provide personalized, actionable diet (nutrition) and exercise (workout) recommendations.
Respond ONLY with a valid JSON object in the following format:
{
  "diet": "detailed diet recommendation markdown",
  "exercise": "detailed exercise recommendation markdown"
}
Do NOT wrap the response in any markdown code block formatting (like \`\`\`json). Return raw JSON only.`;

    const messages = [
      { role: 'system', content: systemPrompt },
      { role: 'user', content: `Here is my report summary data: ${JSON.stringify(reportSummary)}` }
    ];

    const { content } = await processChatMessage(messages, serviceConfig, authenticatedUserId);
    
    // Clean any markdown block wrappers if model adds it anyway
    ...
    try {
      ...
      log('warn', 'Failed to parse JSON response for recommendations, returning raw text:', content);
      return {
        diet: content,
        exercise: "Focus on maintaining your current workout schedule or ask Sparky in chat for specific routines!"
      };
    }
  } catch (error) {
    log('error', `Error generating report recommendations:`, error);
    throw error;
  }
}