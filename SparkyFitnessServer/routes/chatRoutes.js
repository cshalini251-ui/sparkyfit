...
router.post('/recommendations', authenticate, async (req, res, next) => {
  const { reportSummary, service_config } = req.body;
  try {
    const recommendations = await chatService.generateReportRecommendations(reportSummary, service_config, req.userId);
